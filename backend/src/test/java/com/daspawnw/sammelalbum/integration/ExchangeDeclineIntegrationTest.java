...
class ExchangeDeclineIntegrationTest {
        ...
        private void ensureEntities(Long requesterId, Long offererId, Long requestedStickerId, Long offeredStickerId) {
                ...
                if (cardSearchRepository.findByUserIdAndStickerIdIn(requesterId, List.of(requestedStickerId))
                                .isEmpty()) {
                ...
                if (offeredStickerId != null) {
                        ...
                        if (cardOfferRepository.findByUserIdAndStickerIdIn(requesterId, List.of(offeredStickerId))
                                        .isEmpty()) {
                                cardOfferRepository.save(CardOffer.builder()
                                                .userId(requesterId)
                        ...
                        if (cardSearchRepository.findByUserIdAndStickerIdIn(offererId, List.of(offeredStickerId))
                                        .isEmpty()) {
                                cardSearchRepository.save(CardSearch.builder()
                                                .userId(offererId)
                                                .stickerId(offeredStickerId)
        ...
        void declineRequest_ExchangeInterestStatus_ShouldRevertReservations_ExchangeType() throws Exception {
                ...
                CardOffer offererOffer = cardOfferRepository.findByUserIdAndStickerIdIn(offererId, List.of(sticker1Id))
                                .get(0);
                offererOffer.setIsReserved(true);
                cardOfferRepository.save(offererOffer);

                CardSearch requesterSearch = cardSearchRepository
                                .findByUserIdAndStickerIdIn(requesterId, List.of(sticker1Id))
                                .get(0);
                requesterSearch.setIsReserved(true);
                cardSearchRepository.save(requesterSearch);

                CardOffer requesterOffer = cardOfferRepository
                                .findByUserIdAndStickerIdIn(requesterId, List.of(sticker2Id))
                                .get(0);
                requesterOffer.setIsReserved(true);
                cardOfferRepository.save(requesterOffer);

                CardSearch offererSearch = cardSearchRepository
                                .findByUserIdAndStickerIdIn(offererId, List.of(sticker2Id))
                                .get(0);
        ...
        void declineRequest_UnauthorizedUser_ShouldFail() throws Exception {
                ...
                credentialsRepository.save(Credentials.builder()
                                .user(thirdUser).username("thirduser").passwordHash(passwordEncoder.encode("password"))
                                .build());
                ...
                mockMvc.perform(put("/api/exchanges/" + request.getId() + "/decline")
                                ...
                                .andExpect(status().isForbidden()); // Or 403/500 depending on exception handling,
                                                                    // SecurityException
                                                                    // usually 403 or 500
        }

        @Test
        void declineRequest_ByRequester_ShouldSetRequesterCanceledReason() throws Exception {
                ensureEntities(requesterId, offererId, sticker1Id, null);
                ExchangeRequest request = exchangeRequestRepository.save(ExchangeRequest.builder()
                                .requesterId(requesterId)
                                .offererId(offererId)
                ...
                ExchangeRequest updated = exchangeRequestRepository.findById(request.getId()).get();
                assertEquals(ExchangeStatus.EXCHANGE_CANCELED, updated.getStatus());
                assertEquals(CancellationReason.REQUESTER_CANCELED, updated.getCancellationReason());
        }

        @Test
        void declineRequest_ByOfferer_ShouldSetOffererCanceledReason() throws Exception {
                ensureEntities(requesterId, offererId, sticker1Id, null);
                ExchangeRequest request = exchangeRequestRepository.save(ExchangeRequest.builder()
                                .requesterId(requesterId)
                                .offererId(offererId)
                ...
                ExchangeRequest updated = exchangeRequestRepository.findById(request.getId()).get();
                assertEquals(ExchangeStatus.EXCHANGE_CANCELED, updated.getStatus());
                assertEquals(CancellationReason.OFFERER_CANCELED, updated.getCancellationReason());
        }
}
